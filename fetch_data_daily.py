...
def load_json(path, default):
    if os.path.exists(path):
        try:
            ...
            print(f"warning: {path} is corrupted ({e})", file=sys.stderr)
            bak = path + ".bak"
            if os.path.exists(bak):
                try:
                    with open(bak, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    print(f"  recovered from {bak} instead", file=sys.stderr)
                    return data
                except (json.JSONDecodeError, UnicodeDecodeError):
                    pass
            print(f"  no usable backup found either — falling back to empty state. "
                  f"This may cause previously-seen items to be re-scanned/re-fetched.", file=sys.stderr)
    return default
...
def save_json(path, obj):
    if os.path.exists(path):
        try:
            shutil.copyfile(path, path + ".bak")  # best-effort — a power loss mid-write can still corrupt path itself, this is the recovery line
        except OSError:
            pass
    tmp = path + ".tmp"
...
def year_bucket_label(year):
    """5-year windows anchored to CURRENT_YEAR_DEFAULT, going backward: 2022-2026,
    2017-2021, 2012-2016, etc. Full 4-digit years on both ends (not 2-digit endings)
    since a 2-digit ending would look broken across a century boundary, e.g. "1998-02"."""
    offset = CURRENT_YEAR_DEFAULT - year
    bucket_index = offset // YEAR_BUCKET_SIZE
    bucket_end = CURRENT_YEAR_DEFAULT - bucket_index * YEAR_BUCKET_SIZE
    bucket_start = bucket_end - (YEAR_BUCKET_SIZE - 1)
...
class BucketedStore:
    ...
    Existing bucket files are discovered on disk via the filename prefix and loaded once
    at startup into an IMMUTABLE snapshot (so seen-id tracking correctly covers every
    ...
    def refresh_original(self):
        ...
        before starting each new independent collection pass that shares this store
    ...
    def all_records(self):
        out = []
        for records in self.original.values():
            out.extend(records)
        return out

    def apply_full_collected(self, collected_so_far):
        """collected_so_far is the FULL accumulated list from a fetch pass so far (not
        an incremental delta) — safe to call repeatedly as it grows. Returns
        {label: full_record_list} only for buckets that have something new."""
        by_label = {}
        for rec in collected_so_far:
            label = year_bucket_label(rec["year"])
            by_label.setdefault(label, []).append(rec)
        return {label: self.original.get(label, []) + new_recs for label, new_recs in by_label.items()}
    ...
    def total_on_disk(self):
        ...
        all saves for this run have already happened."""
        total = 0
        for path in glob.glob(f"{self.prefix}[0-9]*.json"):
...
def evaluate_show(key, s, min_votes, special_benchmark, standup_benchmark):
    ...
    if is_czech_or_slovak(s):
        if not (set(countries) & CZSK_COUNTRY_CODES):
            return (False, False, countries)  # original_language says cs/sk, but no CZ/SK origin_country — likely bad TMDb metadata
        if vote_count < CZSK_MIN_VOTES_FOR_SCORE:
    ...
    for year in years:
        ...
        while page <= max_pages:
            ...
            for g in results:
                ...
                if not is_released(g["released"], today):
                    continue
                if game_wishlist_count(g) < min_wishlist:
                    continue
                seen_ids.add(g["id"])
                try:
                    details = get_game_details(key, g)
...
def refresh_recent_unranked_games(key, store, today, days=7):
    """Games added in the last N days that came in unranked (score=null) often just
    needed more time — a Metacritic review can land days later, or RAWG's own rating
    count can cross the tier threshold once more players have rated it. This re-checks
    those specific games against RAWG's current data and upgrades their score in place
    if they now qualify — never re-adds or duplicates anything, purely an update."""
    ...
    for label, records in list(store.original.items()):
        changed = False
        for g in records:
            if g.get("score") is not None or g.get("date", "") < cutoff:
                continue
            checked += 1
            try:
                details = get_game_details(key, {"id": g["rawg_id"]})
            except Exception as e:
                print(f"  warning: could not re-check '{g.get('title')}' ({e})", file=sys.stderr)
                continue
            score_input = {
                "name": g.get("title"), "metacritic": details.get("metacritic"),
                "released": g.get("date"), "ratings_count": details.get("ratings_count"),
                "rating": details.get("rating"), "genres": details.get("genres") or [],
                "platforms": details.get("platforms") or [],
            }
            new_score = game_score(score_input)
...
def main():
    ...
    ap.add_argument("--movies-prefix", default="movies", help="worldwide (non-Czech/Slovak) movies are split across files named {prefix}{start}-{end}.json, e.g. movies2022-2026.json")
    ap.add_argument("--movies-czsk-prefix", default="movies_czsk", help="Czech/Slovak-original movies, same year-bucketed file scheme, kept separate from the worldwide files")
    ap.add_argument("--shows-prefix", default="shows", help="TV shows, same year-bucketed file scheme (Czech/Slovak shows stay mixed in here — only movies are split by language)")
    ap.add_argument("--games-prefix", default="games", help="games, same year-bucketed file scheme")
    ap.add_argument("--state-file", default="fetch_state.json")
    ap.add_argument("--skip-movies", action="store_true", help="skip movies entirely, including Czech/Slovak (e.g. to just refresh games without re-scanning through movies first)")
    ap.add_argument("--skip-worldwide-movies", action="store_true", help="skip only the general/worldwide (all-language) movie scan, while still fetching Czech/Slovak movies")
    ...
    movies_store = BucketedStore(args.movies_prefix)
    ...
    game_seen = set(state.get("game_seen", [])) | {g["rawg_id"] for g in games_store.all_records() if g.get("rawg_id")}

    def is_czsk_record(rec):
        return rec.get("original_language") in ("cs", "sk")

    def make_movie_checkpoint(seen_set, seen_key):
        """Splits newly-collected movies both by language (worldwide vs Czech/Slovak)
        AND by year bucket on every checkpoint — a single run can easily span more than
        one 5-year bucket (e.g. scanning from 2019 down to 2015 touches both the
        2017-2021 and 2012-2016 buckets)."""
        movies_store.refresh_original()
        movies_czsk_store.refresh_original()
        def checkpoint(collected_so_far=None):
            ...
            movies_store.save(movies_store.apply_full_collected(new_worldwide))
            movies_czsk_store.save(movies_czsk_store.apply_full_collected(new_czsk))
            state[seen_key] = list(seen_set)
            save_json(args.state_file, state)
            save_json(args.people_cache_file, people_cache)
        return checkpoint

    def make_bucket_checkpoint(store, seen_set, seen_key):
        store.refresh_original()
        def checkpoint(collected_so_far=None):
            store.save(store.apply_full_collected(collected_so_far or []))
            state[seen_key] = list(seen_set)
            save_json(args.state_file, state)
            save_json(args.people_cache_file, people_cache)
        return checkpoint
    ...
    if args.skip_movies:
        ...
        try:
            ...
            print(f"movies: could not fetch genre map ({e})", file=sys.stderr)

        if args.skip_worldwide_movies:
            print("Skipping worldwide movie scan (--skip-worldwide-movies) — only Czech/Slovak movies will be fetched.", file=sys.stderr)
        else:
            print("Fetching movies...", file=sys.stderr)
            try:
                if not args.skip_backfill:
                    cp = make_movie_checkpoint(movie_seen, "movie_seen")
                    fetch_movie_window(args.tmdb_key, state["movie"], args.years_per_run, args.floor_year, args.min_votes, movie_genres, movie_seen, args.galleries, args.fetch_reviews, imdb_ratings, args.min_imdb_votes, args.anime_benchmark_votes, args.standup_benchmark_votes, people_cache, today, cp)
            except Exception as e:
        ...
        for lang_code in ("cs", "sk"):
            ...
            try:
                if not args.skip_backfill:
                    cp = make_movie_checkpoint(movie_seen, "movie_seen")
                    fetch_lang_movie_window(args.tmdb_key, state[state_key], args.years_per_run, args.floor_year, lang_code, movie_genres, movie_seen, args.galleries, args.fetch_reviews, imdb_ratings, args.min_imdb_votes, args.anime_benchmark_votes, args.standup_benchmark_votes, people_cache, today, cp)
            except Exception as e:
    ...
    if args.skip_shows:
        ...
        try:
            ...
            if not args.skip_backfill:
                ...
                fetch_show_window(args.tmdb_key, state["show"], args.years_per_run, args.floor_year, args.min_votes, show_genres, show_seen, args.galleries, args.fetch_reviews, imdb_ratings, args.min_imdb_votes, args.anime_benchmark_votes, args.standup_benchmark_votes, people_cache, today, cp)
        except Exception as e:
        ...
        for lang_code in ("cs", "sk"):
            ...
            try:
                if not args.skip_backfill:
                    cp = make_bucket_checkpoint(shows_store, show_seen, "show_seen")
                    fetch_lang_show_window(args.tmdb_key, state[state_key], args.years_per_run, args.floor_year, lang_code, show_genres, show_seen, args.galleries, args.fetch_reviews, imdb_ratings, args.min_imdb_votes, args.anime_benchmark_votes, args.standup_benchmark_votes, people_cache, today, cp)
            except Exception as e:
    ...
    if args.skip_games:
        ...
        try:
            if not args.skip_backfill and not args.skip_games_backfill:
                cp = make_bucket_checkpoint(games_store, game_seen, "game_seen")
                fetch_game_window(args.rawg_key, state["game"], args.years_per_run, args.floor_year, game_seen, today, cp)
        except Exception as e:
        ...
        try:
            refresh_recent_unranked_games(args.rawg_key, games_store, today)
        except Exception as e:
            print(f"games (score refresh): unexpected error ({e})", file=sys.stderr)
        print(f"  -> saved. games total so far: {games_store.total_on_disk()}", file=sys.stderr)

    def write_manifest():
        manifest = sorted(
            os.path.basename(p) for prefix in (args.movies_prefix, args.movies_czsk_prefix, args.shows_prefix, args.games_prefix)
            for p in glob.glob(f"{prefix}[0-9]*.json")
        )
